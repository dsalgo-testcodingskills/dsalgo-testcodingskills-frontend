...
const PlansTab = ({ subDetails, onUpdate, planLimits }) => {
  ...
  const activeSub =
    subDetails &&
    subDetails.length > 0 &&
    subDetails[0].status === SUBSCRIPTION_STATUS.ACTIVE
      ? subDetails[0]
      : null;
  const cancelledSub =
    subDetails &&
    subDetails.length > 0 &&
    subDetails[0].status === SUBSCRIPTION_STATUS.CANCELLED
      ? subDetails[0]
      : null;
  const canResubscribe =
    cancelledSub && Date.now() > cancelledSub.current_end * 1000;

  const subscriptionEndDate = cancelledSub?.current_end
    ? moment(cancelledSub.current_end * 1000)
    : null;
  const isSubscriptionExpired = subscriptionEndDate
    ? subscriptionEndDate.isBefore(moment(), "day")
    : false;
  ...
  const handleSubscribeClick = () => {
    ...
    if (cancelledSub && !canResubscribe) {
      setInfoOpen(true);
      return;
    }
    setTermsOpen(true);
  ...
  const confirmSubscription = async (planId) => {
    ...
    try {
      ...
      if (!res) {
        toast(
          <CustomToast type="error" message="Razorpay SDK failed to load." />,
        );
        return;
      }
  ...
  const paidLimits = planLimits?.paid || {
    tests: 100,
    users: 10,
    customQuestions: 20,
    amount: 1000,
  ...
  return (
    <div className="plans-view">
      ...
      <div className="plans-grid">
        ...
        <UICard className="plan-card" featured={true}>
          ...
          <div className="plan-price">
            ₹{paidLimits.amount?.toLocaleString() || "0"}{" "}
            <span>{SUBSCRIPTION_STRINGS.PLANS.PER_MONTH}</span>
          </div>
          <div className="plan-cycle">
            ...
            <UITooltip
              text={`Your card will be charged ₹${
                paidLimits.amount?.toLocaleString() || "0"
              } automatically each month. Cancel anytime.`}>
              <span
                style={{
          ...
          <div
            ...
            <button
              ...
              {activeSub
                ? "Active Plan"
                : `${
                    SUBSCRIPTION_STRINGS.PLANS.SUBSCRIBE
                  } — ₹${paidLimits.amount.toLocaleString()} ${
                    SUBSCRIPTION_STRINGS.PLANS.PER_MONTH
                  }`}
            </button>
          </div>
          <p
            ...
            Total commitment: ₹{(paidLimits.amount * 12).toLocaleString()} ·
      ...
      <UICallout variant="info">
        {SUBSCRIPTION_STRINGS.PLANS.AUTO_DEBIT_HELP} You can cancel anytime;
      ...
      <Dialog open={infoOpen} onClose={() => setInfoOpen(false)}>
        <div
          style={{ padding: "24px", maxWidth: "360px", textAlign: "center" }}>
          <i
          ...
          <p
            ...
            {isSubscriptionExpired ? (
              <>
                Your cancelled subscription access ended on{" "}
                <strong>
                  {subscriptionEndDate?.format("DD MMM YYYY") || "-"}
                </strong>
                .
              </>
            ) : (
              <>
                You have a cancelled subscription that is still active until{" "}
                <strong>
                  {subscriptionEndDate?.format("DD MMM YYYY") || "-"}
                </strong>
                .
